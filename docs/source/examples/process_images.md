...
If there's no clean peak to fit a curvature against, pass `curvature=None`
(the default) to `get_spectrum`/`get_spectra`/`process_rxes`/
`process_rxes_mcd` — this skips the correction entirely and simply
integrates each image in the vertical direction.
