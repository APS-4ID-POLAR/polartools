...
def show_meta(db,scan_from,scan_to=None,query=None,long=False):
    ...
    for scan_number in range(scan_from,scan_to):        
        ...
        if scan_type=='list_scan':
            print(f'#{scan_number} {scan_type}[{scan_type2}] {motors} {scan_from} {scan_to} {number_of_points} {status}')
        else:
            print(f'#{scan_number} {scan_type} {motors} {scan_from} {scan_to} {number_of_points} {status}')
