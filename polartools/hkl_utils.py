...
def setor0(delta=None,th=None,chi=None,phi=None,gamma=None,mu=None,h=None,k=None,l=None):
    ...
    orienting_refl = sample._orientation_reflections
    if not delta and not th and not chi and not phi and not gamma and not mu and not h and not k and not l:
...
def setor1(delta=None,th=None,chi=None,phi=None,gamma=None,mu=None,h=None,k=None,l=None):
    samples = [_geom_.calc._sample]
    sample = samples[0]
    ref=sample._sample.reflections_get()
    orienting_refl = sample._orientation_reflections
...
def set_orienting():
    ...
    if not or1:
        or1 = or1_old
    sample._orientation_reflections.pop(0)
    sample._orientation_reflections.insert(0,sample._sample.reflections_get()[int(or0)])
    sample._orientation_reflections.pop(1)
    sample._orientation_reflections.insert(1,sample._sample.reflections_get()[int(or1)])
    print("Computing UB!")
    sample.compute_UB(sample._orientation_reflections[0],sample._orientation_reflections[1])
    _geom_.forward(1,0,0)
...
def ca(h,k,l):
    ...
    print("\n   H K L = {:5f} {:5f} {:5f}".format(h,k,l,))
    print("\n   Lambda (Energy) = {:6.4f} \u212B ({:6.4f} keV)".format(_geom_.calc.wavelength, _geom_.calc.energy))
...
def wh():
    ...
    print("\n   Lambda (Energy) = {:6.4f} \u212B ({:6.4f} keV)".format(_geom_.calc.wavelength, _geom_.calc.energy))
    print("\n   {:8}{:8}{:8}{:8}{:8}{:8}".format("Delta","Theta", "Chi", "Phi", "Gamma", "Mu"))
    print("{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}".format(_geom_.delta.get()[0],_geom_.omega.get()[0],_geom_.chi.get()[0],_geom_.phi.get()[0],_geom_.gamma.get()[0],_geom_.mu.get()[0],))
...
def setlat(a=None,b=None,c=None,alpha=None,beta=None,gamma=None):
    ...
    if not c:
        c = lattice[2]
    alpha = input("Lattice alpha ({})? ".format(lattice[3])) if not alpha else alpha
    if not alpha:
        alpha = lattice[3]
    beta = input("Lattice beta ({})? ".format(lattice[4])) if not beta else beta
    if not beta:
        beta = lattice[4]
    gamma = input("Lattice gamma ({})? ".format(lattice[5])) if not gamma else gamma
    if not gamma:
        gamma = lattice[5]
